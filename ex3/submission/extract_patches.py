...
def extract_patches(img, p):
    ...
    """
    #idea è di rollare la matrice al posto di iterare su di essa
    #rollo per ogni riga colonna nel patch 
    #faccio stack di tutte le immagini
    #in questo modo ottengo tutti i neighbohrs

    #devo definire le immagini da rollare 
    rolls = []
    for i in range(p):
        for j in range(p):
            rolls.append((j - 1, i - 1))
    rolls.reverse()
    shifted_versions = []
    for roll in rolls:
        rolled = np.roll(img, roll, axis=(1, 0))
        shifted_versions.append(rolled)
    out = np.concatenate(shifted_versions, axis=2)
    return out 
