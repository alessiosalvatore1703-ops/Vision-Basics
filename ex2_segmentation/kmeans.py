...
def kmeans_fit(data, k, rng, n_iter=500, tol=1.e-4):
    ...
    iteration = 0
    while(change > tol and iteration < n_iter):
        iteration += 1 
        #determine points in each cluster(for each sample find
        #the closest ci --> put in to his cluster)
        distances = compute_distance(data, centroids) 
        #row distances --> samples
        #colums are clusters
        #given points in ci solve for ci --> compute the mean
        assignments = np.argmin(distances, axis=1)
        new_centroids = np.zeros((k, P))  
        #compute mean 
        counter = np.zeros(k)
        for i in range(N):
            counter[assignments[i]] += 1
            new_centroids[assignments[i]] += data[i, :]
        non_empty = counter > 0
        new_centroids[non_empty] /= counter[non_empty, np.newaxis]
        #if change = max(delta centroids)
        change = np.max(np.linalg.norm(centroids - new_centroids, axis=1))
        if change < tol:
...
def compute_distance(data, clusters):
    ...
    """
    data_expanded = data[:, np.newaxis, :]  # (n_samples, 1, n_features)
    clusters_expanded = clusters[np.newaxis, :, :]  # (1, n_clusters, n_features)
    distance = np.linalg.norm(data_expanded - clusters_expanded, axis=2)
    return distance
...
def kNN(data_train, labels_train, data_test, k):
    ...
    labels_pred = np.zeros(n_test, dtype=int)
    
    # Per ogni punto di test
    for i in range(n_test):
        # Calcola distanze da tutti i punti di training
        test_point = data_test[i:i+1, :]  # Shape (1, n_features)
        distances = compute_distance(test_point, data_train)  # Shape (1, n_train)
        distances = distances[0]  # Shape (n_train,)
        
        # Trova i k vicini più prossimi
        nearest_indices = np.argsort(distances)[:k]
        
        # Prendi le label dei k vicini
        nearest_labels = labels_train[nearest_indices]
        
        # Voto di maggioranza (per k=1 è semplicemente la label del più vicino)
        # Per k > 1, conta quale label appare più spesso
        unique, counts = np.unique(nearest_labels, return_counts=True)
        labels_pred[i] = unique[np.argmax(counts)]
